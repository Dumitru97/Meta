...
namespace Objects {
  // O(nm)
	std::string ObjMovedByActFuncs(int obj_idx, int act_idx) {
		...
		// Constructing format
		std::string fmt =
		"TEST_INLINE void Objs{0}MovedByActs{1}(Objects{0}& objs{0}, const Actors{1}& acts{1}, OObject{0}Moved, OActor{1}UpdateDone) {{" nl;
...
//		fmt +=
//			R"(
//	const auto& act = acts{1}[bounded_rand(acts{1}.size())];
//		for (int i = 0; i < objs{0}.size(); ++i) {{
//			const auto& obj = objs{0}[i].obj;
//			auto& obj_next = objs{0}[i].obj_next;
//
